...
image: https://images.iminling.com/app/hide.php?key=SlBudmJhK1kyNkJ5NGtnVm53c1kwOUpBdFNJd1I2dHR5NXk5VjM1QkNsN1VtWm9DVWNJdi9jdEEyL0o0eTZDTXJPNUhlT3c9
categories:
  - wordpress
tags:
  - git-it-write
...
Git-it-write插件在Wordpress的插件库是可以找到的，直接搜索安装就可以了。安装后可以在设置中找到对这个插件的配置。
...
点击左上的`add a new repository to publish posts form`按钮来添加一个新的仓库：
...
来到`github`的`settings`页面：登录`github`后，点击右上角的头像，就会看到有`settings`选项。进入`settings`后，拉到最下边找到`developer settings`选项，如下图：
...
设置完成后就可以生成token了，把生成的tokent填写在`General settings`处了：
...
在我们写完markdown文件后，push到上边配置的git仓库中，下边就可以来进行发布操作了。
...
在git it write的配置页面可以看到webhook secret的配置选项，没错，这个就是和github的webhook进行联动的，需要我们在github中进行相关的配置。
...
github中添加webhook如下，需要填写url，content type以及secret，这里的secret和git it write中的是同一个。另外需要什么时候来触发这个webhook,我这里选择`Just the push event`.其他的大家根据需求来进行填写。
