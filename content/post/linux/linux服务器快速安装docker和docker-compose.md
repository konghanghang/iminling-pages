...
description: 记录一下安装docker的过程，具体的安装步骤都是参考docker的官方网站进行的安装。 ubuntu安装 卸载旧版 执行下边的命令 执行上边的命令清除以前的安装。 设置源 执行以下命令： 设置安装源。 安装 先更新刚才安装的源，然后就可以执行安装了： 上边的安装是安装最新版本，如果想安装特定版本则需要先查看有哪些可以安装的版本： 找到版本后执行以下命令来进行安装： 安装后就可以使用了。
